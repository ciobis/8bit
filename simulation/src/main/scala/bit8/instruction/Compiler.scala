...
object Compiler {
  ...
  private val MovVarToMemory = "MOV \\[MX\\],(.*)".r
  private val MovMemoryToVar = "MOV (.*),\\[MX\\]".r
  ...
  private def getCompilable(line: String): Option[CompilableInstruction] = line match {
    ...
    case MovVarToMemory(label) => Some(CompilableInstruction(Instruction.MovVarToMemory, (_, vars) => 0 +: splitIntToBytes(vars(label))))
    case MovMemoryToVar(label) => Some(CompilableInstruction(Instruction.MovMemoryToVar, (_, vars) => 0 +: splitIntToBytes(vars(label))))
